...
SELECT MAX (maas) FROM calisanlar3 WHERE maas <(SELECT MAX(maas) FROM calisanlar3)

--Interview Question:calisanlar3 tablosunda ikinci en yüksek maaşı alan çalışanın bilgilerini gösteriniz.ÖDEV
SELECT*FROM calisanlar3 WHERE maas = (SELECT MAX(maas) FROM calisanlar3 WHERE maas<(SELECT MAX(maas) FROM calisanlar3))

--calisanlar3 tablosunda max veya min maaşı alan çalışanların
-- tüm fieldlarını gösteriniz.
SELECT * FROM calisanlar3 WHERE maas=
(SELECT MAX(maas) FROM calisanlar3) OR maas=(SELECT MIN(maas)FROM calisanlar3)

-- Ankara'da calisani olan markalarin marka id'lerini ve calisan sayilarini listeleyiniz.
SELECT * FROM markalar WHERE marka_ismi IN
('Pierre cardin','Vakko','Adidas' )

--2.doğru yol
SELECT marka_id,calisan_sayisi FROM markalar WHERE marka_isim IN 
(SELECT isyeri FROM calisanlar3 WHERE sehir='Ankara')
...
--SELECT DISTINCT(sehir) FROM calisanlar3;-- tekrarsız alır
