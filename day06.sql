...
--NOT: group by ile gruplama yapıldığında SELECT den sonra sadece grouplanan sütun adı veya 
--herhangi bir sütünun AGGREGATE fonskiyon ile kullanımı mümkündür.

SELECT isim, SUM(urun_miktar)
From manav
GROUP BY isim

--NOT: GROUP BY dan sonraki sütun sırası önemli, ilk sütundan başlayarak gruplar, AGGREGATE fonksiyon kullanıldığında 
...
--30-HAVING clause: GROUP BY komutundan sonra filtrelemek için AGGREGATE fonksiyon ile birlikte kullanılır
...
--NOT: gruplama yaptıktan sonra sonuçları filtrelemek için HAVING AGGREGATE fonksiyon ile
...
SELECT sehir,COUNT(id)calisan_personel
FROM personel
GROUP BY sehir
HAVING COUNT(id) >1
...
--31-UNION: iki farklı sorgu sonucunda  birleştirerek  görüntülemeyi sağlar
--           tekrar eden veriler var ise tekrarlı olanları bir kere gösterir tekrar edenleri göstermez
...
SELECT salary AS naas_yas,prog_lang FROM developers WHERE prog_lang='Java'
UNION
...
--33--EXCEPT:bir sorgunun sonuçlarından diğer bir sorgunun sonuçları hariç olarak gösterir
