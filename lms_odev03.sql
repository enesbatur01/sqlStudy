...
SELECT course_name,credit FROM courses WHERE course_name IN (SELECT course_name FROM teachers WHERE age<35 )

--Her bir şehirde verilen derslerin isimlerini ve kredisini listeleyiniz. 
SELECT course_name,credit FROM courses WHERE course_name IN (SELECT course_name FROM teachers )
...
--İstanbulda yaşayan öğretmenlerin isimlerini(firstname), verdiği derslerin isimlerini ve bitiş tarihini listeleyiniz. 
...
SELECT SUM(salary) FROM teachers WHERE course_name IN(SELECT course_name FROM courses WHERE start_date BETWEEN '01-02-2023' AND '01-05-2023')
...
SELECT course_name,credit,
					(SELECT MAX(salary) FROM teachers t WHERE t.course_name=c.course_name ),
...
SELECT course_name, start_date FROM courses WHERE course_name IN(SELECT course_name FROM teachers WHERE age>35)

--Hiçbir öğretmen tarafından verilmeyen dersleri isim, başlangıç ve bitiş tarihlerini listeleyiniz.

SELECT course_name,start_date,finish_date 
FROM courses c 
WHERE NOT EXISTS (SELECT * FROM teachers t WHERE t.course_name=c.course_name)

--En az bir öğretmen tarafından verilen derslerin ücretlerini 10 artırınız.
UPDATE courses c SET course_fee = course_fee+10 WHERE EXISTS(SELECT * FROM teachers t WHERE t.course_name=c.course_name)

--Ücreti 170 den fazla olan ders varsa bu dersi veren öğretmenlerin tüm bilgilerini listeleyiniz.(IN kullanmayınız.)
SELECT *
FROM teachers t
WHERE EXISTS (SELECT * FROM courses c WHERE course_fee>170 AND c.course_name=t.course_name )

--Başlangıç tarihi '03-11-2022' olan kursun ismini ' Core' ekleyiniz.
UPDATE courses SET course_name= course_name ||' Core' WHERE start_date='03-11-2022'

--Maaşı ortalama maaştan yüksek olan öğretmenin şehrini Antalya olarak değiştiriniz.
UPDATE teachers SET city = 'Antalya' WHERE salary > (SELECT AVG(salary) FROM teachers)

--Yaşı en küçük olan dersin öğretmeninin maaşını 2 katına çıkarınız.
UPDATE teachers SET salary = salary*2 WHERE age =(SELECT MIN(age)FROM teachers)

--En erken başlayan kursun kredisini 2 artırınız.
UPDATE courses SET credit = credit+2 WHERE start_date =(SELECT MIN(start_date)FROM courses)
