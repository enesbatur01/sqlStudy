...
--26-UPDATE .. 
	...
	--WHERE koşul            : koşulu sağlayan satırlardaki dataları değiştirir yada günceller
...
UPDATE calisanlar3 SET isyeri = (SELECT isyeri FROM calisanlar3 WHERE isim='Veli Yıldırım') WHERE isim= 'Ali Seker'
...
UPDATE calisanlar3 SET sehir=sehir||' Şubesi' WHERE isyeri='Vakko'

--alternatif

UPDATE calisanlar3 SET sehir=CONCAT (sehir,' Şubesi') WHERE isyeri='Vakko'
...
UPDATE people SET address='MISSING...' WHERE address IS NULL


--28-ORDER BY : kayıtları belirli bir veya daha fazla sütuna göre 
--varsayılan olarak NATURAL(artan yönde,ASCENDING) olarak sıralamamızı sağlar
CREATE TABLE person
(
ssn char(9),
isim varchar(50),
...
SELECT * FROM person ORDER BY adres ASC--okunabilirlik artar yazmak zorunda değiliz

--person tablosundaki tüm kayıtları soyisime göre (azalan) sıralayarak listeleyiniz.

SELECT * FROM person ORDER BY soyisim DESC-- azalan şekilde sıralar
...
SELECT * FROM person WHERE soyisim='Bulut' ORDER BY 2 DESC --not recommended: tavsiye edilmez sütunun adı gözükmediği için tavsiye edilmez
...
SELECT isim,soyisim,LENGTH(soyisim)AS uzunluk FROM person ORDER BY LENGTH(soyisim)
...
SELECT isim,sehir,maas FROM calisanlar3 WHERE maas> (SELECT AVG(maas)FROM calisanlar3)ORDER BY maas
